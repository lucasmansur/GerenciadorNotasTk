...
def onSelected(event):
    ...
    lblTitle = Label(registerTitle, text="Atualizar dados",font=("consolas", 25), bg="#7cbb63", width=300)
...
def submitData():
    if grade.get() == None or subject.get() == "" or grade2.get() == None:
        result = msgb.showwarning("Campo vazio!", "Você esqueceu de preencher algum campo. Porfavor, verifique os campos!", icon="warning")
    else:
        tree.delete(*tree.get_children())
        average.set((grade.get()+grade2.get())/2)
